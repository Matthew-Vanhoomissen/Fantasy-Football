...
def create_year_data(num):
    ...
    all_data = pd.read_csv(f"data/play_by_play/play_by_play_202{num}.csv", low_memory=False)

    players = pd.read_csv(f"data/player_names/players_202{num}.csv", header=None, skiprows=1, low_memory=False)[0].dropna().unique().tolist()
    ...
    if num != 5:
        ...
        final_data.to_csv(f"data/training_dataset/training_dataset_202{num}.csv", index=False)
